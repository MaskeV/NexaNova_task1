...
const createBlankTimeSlots = () => {
  ...
  days.forEach(day => {
    timeSlots.forEach(time => {
      slots.push({ day, timeSlot: time, subject: null, trainer: null, isAllocated: false });
    });
  });
  return slots;
};
...
const allocateSlot = async (req, res) => {
  try {
    ...
    if (slot.isAllocated) {
      return res.status(400).json({ success: false, message: 'Time slot is already allocated' });
    }

    // Verify trainer exists by empId (string, NOT ObjectId)
    const trainer = await Trainer.findOne({ empId: trainerId });
    if (!trainer) {
      return res.status(404).json({ success: false, message: 'Trainer not found' });
    }

    // Verify subject exists by subjectId (string, NOT ObjectId)
...
const getScheduleByWeek = async (req, res) => {
  try {
    ...
    // ✅ Plain findOne — NO .populate() because trainer/subject are strings, not ObjectIds
    const schedule = await Schedule.findOne({ weekId });
    ...
    // Manually populate subject and trainer details using their string IDs
    const populatedSlots = await Promise.all(
      schedule.timeSlots.map(async (slot) => {
        ...
        if (slot.trainer) {
          // ✅ Query by empId (string), NOT by _id — avoids the CastError
