...
// @desc    Allocate trainer and module to a time slot
// @route   PUT /api/schedule/slot/:slotId
// @access  Admin
const allocateSlot = async (req, res) => {
  try {
    const { slotId } = req.params;
    const { weekId, trainerId, moduleId } = req.body;
    
    // Validation
    if (!weekId || !trainerId || !moduleId) {
      return res.status(400).json({
        success: false,
        message: 'Week ID, trainer ID, and module ID are required'
      });
    }
    ...
    const module = await Subject.findOne({ subjectId: moduleId });
    if (!module) {
      return res.status(404).json({
        success: false,
    ...
    // Check if trainer teaches this module
    if (!trainer.subjects.includes(moduleId)) {
      return res.status(400).json({
        success: false,
        message: `Trainer ${trainer.name} does not teach ${module.name}`
      });
    }
...
const getScheduleByWeek = async (req, res) => {
  try {
    ...
    // Populate module details manually (since it's a string reference)
